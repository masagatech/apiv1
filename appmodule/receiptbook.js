...
rb.getAllRBI = function getAllRB(req, res, done) {
    db.callProcedure("select " + globals.schema("funget_recptbkissued") + "($1,$2,$3::json);", ['rb', 'rb1', req.body], function(data) {
        rs.resp(res, 200, data.rows);
    }, function(err) {
        rs.resp(res, 401, "error : " + err);
    }, 2)
...
rb.getRBIDetails = function getRBDetails(req, res, done) {
    db.callProcedure("select " + globals.schema("funget_recptbkissued") + "($1,'a',$2::json);", ['rb', req.body], function(data) {
        rs.resp(res, 200, data.rows);
    }, function(err) {
        rs.resp(res, 401, "error : " + err);
    }, 1)
...
rb.saveRBIDetails = function saveRBIDetails(req, res, done) {
    db.callFunction("select " + globals.schema("funsave_recptbkissued") + "($1::json);", [req.body], function(data) {
        rs.resp(res, 200, data.rows);
    }, function(err) {
        rs.resp(res, 401, "error : " + err);
    })
